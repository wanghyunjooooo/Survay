...
function Home() {
    ...
    const fetchJoinedSurveys = async () => {
        try {
            console.log("참여한 설문 API 호출 시작"); // ✅ 확인용
            const res = await getMyParticipatedSurveys();
            console.log("참여한 설문 API 응답:", res); // ✅ 확인용
            ...
            if (res.success) {
                ...
                console.log("파싱 후 surveys:", surveys); // ✅ 확인용
                setJoinedSurveys(surveys);
            } else {
                alert("참여한 설문 리스트 불러오기 실패: " + res.message);
            }
        } catch (err) {
            console.error("참여한 설문 조회 오류:", err);
            alert("참여한 설문 리스트 조회 중 오류가 발생했습니다.");
        }
    };
