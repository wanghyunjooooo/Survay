...
// 내가 참여한 설문 조회
// ================================
export const getMyParticipatedSurveys = async () => {
    try {
        const res = await axios.get(
            "http://localhost:8080/api/surveys/my-participated",
            {
                headers: {
        ...
        return res.data; // { success: true, surveys: [...] }
    } catch (err) {
        console.error("참여한 설문 API 오류:", err);
        return { success: false, surveys: [], message: err.message };
    }
};
export default api;
